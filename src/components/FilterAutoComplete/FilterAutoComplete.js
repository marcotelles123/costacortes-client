...
export default function FilterAutoComplete(props) {
  ...
  function nameContains(value) {
    ...
    for (i = 0; i < productsDB.length; i++) {
      if (productsDB[i].name.toLowerCase().includes(value.toLowerCase())) {
        filteredProducts.push(productsDB[i]);
      }
    }
  ...
  return (
    <Autocomplete
      ...
      onChange={(event, value) =>
        nameContains(value === null ? "" : value.name)
      }
      renderInput={params => (
        <TextField
